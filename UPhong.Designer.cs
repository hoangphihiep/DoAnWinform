namespace DuLich
{
    partial class UPhong
    {
        /// <summary> 
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary> 
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Component Designer generated code

        /// <summary> 
        /// Required method for Designer support - do not modify 
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            lbl_ChiDemSoLuong = new Label();
            lbl_SoLuongNguoi = new Label();
            lbl_SoGiuong = new Label();
            lbl_SucChua = new Label();
            txt_SucChua = new TextBox();
            lbl_m2 = new Label();
            lbl_KichThuoc = new Label();
            txt_KichThuoc = new TextBox();
            label5 = new Label();
            lbl_ChiTietPhong = new Label();
            lbl_ThietLap = new Label();
            lbl_Nguoi = new Label();
            txt_tienThemKhach = new TextBox();
            lbl_ThemKhach = new Label();
            lbl_VND = new Label();
            txt_GiaToiThieu = new TextBox();
            lbl_GiaToiThieuMoiDem = new Label();
            lbl_GiaGocChoPhongTieuChuan = new Label();
            lbl_GiaMoiDem = new Label();
            txt_SoGiuong = new TextBox();
            lbl_ThemHinhAnh = new Label();
            btn_ThemPhong = new Button();
            ptb_AnhPhong = new PictureBox();
            lbl_TenPhong = new Label();
            txt_TenPhong = new TextBox();
            txt_SoPhong = new TextBox();
            lbl_SoPhong = new Label();
            btn_ChinhSua = new Button();
            btn_Next = new Button();
            btn_Back = new Button();
            ((System.ComponentModel.ISupportInitialize)ptb_AnhPhong).BeginInit();
            SuspendLayout();
            // 
            // lbl_ChiDemSoLuong
            // 
            lbl_ChiDemSoLuong.AutoSize = true;
            lbl_ChiDemSoLuong.Font = new Font("Segoe UI", 9F, FontStyle.Regular, GraphicsUnit.Point);
            lbl_ChiDemSoLuong.Location = new Point(17, 407);
            lbl_ChiDemSoLuong.Name = "lbl_ChiDemSoLuong";
            lbl_ChiDemSoLuong.Size = new Size(399, 20);
            lbl_ChiDemSoLuong.TabIndex = 87;
            lbl_ChiDemSoLuong.Text = "Chỉ đếm số lượng giường của quý đối tác cho chỗ nghỉ này\r\n";
            lbl_ChiDemSoLuong.Click += lbl_ChiDemSoLuong_Click;
            // 
            // lbl_SoLuongNguoi
            // 
            lbl_SoLuongNguoi.AutoSize = true;
            lbl_SoLuongNguoi.Font = new Font("Segoe UI", 9F, FontStyle.Regular, GraphicsUnit.Point);
            lbl_SoLuongNguoi.Location = new Point(14, 288);
            lbl_SoLuongNguoi.Name = "lbl_SoLuongNguoi";
            lbl_SoLuongNguoi.Size = new Size(559, 40);
            lbl_SoLuongNguoi.TabIndex = 86;
            lbl_SoLuongNguoi.Text = "Số lượng người tối đa có thể ngủ thoải mái theo số lượng giường và sofa cung cấp\r\n\r\n";
            // 
            // lbl_SoGiuong
            // 
            lbl_SoGiuong.AutoSize = true;
            lbl_SoGiuong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_SoGiuong.Location = new Point(15, 377);
            lbl_SoGiuong.Name = "lbl_SoGiuong";
            lbl_SoGiuong.Size = new Size(103, 23);
            lbl_SoGiuong.TabIndex = 85;
            lbl_SoGiuong.Text = "Số giường : \r\n";
            // 
            // lbl_SucChua
            // 
            lbl_SucChua.AutoSize = true;
            lbl_SucChua.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_SucChua.Location = new Point(15, 252);
            lbl_SucChua.Name = "lbl_SucChua";
            lbl_SucChua.Size = new Size(95, 23);
            lbl_SucChua.TabIndex = 84;
            lbl_SucChua.Text = "Sức chứa : ";
            // 
            // txt_SucChua
            // 
            txt_SucChua.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_SucChua.Location = new Point(14, 332);
            txt_SucChua.Name = "txt_SucChua";
            txt_SucChua.Size = new Size(594, 30);
            txt_SucChua.TabIndex = 83;
            // 
            // lbl_m2
            // 
            lbl_m2.AutoSize = true;
            lbl_m2.BackColor = SystemColors.Window;
            lbl_m2.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_m2.Location = new Point(574, 216);
            lbl_m2.Name = "lbl_m2";
            lbl_m2.Size = new Size(34, 23);
            lbl_m2.TabIndex = 82;
            lbl_m2.Text = "m2";
            // 
            // lbl_KichThuoc
            // 
            lbl_KichThuoc.AutoSize = true;
            lbl_KichThuoc.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_KichThuoc.Location = new Point(15, 172);
            lbl_KichThuoc.Name = "lbl_KichThuoc";
            lbl_KichThuoc.Size = new Size(106, 23);
            lbl_KichThuoc.TabIndex = 81;
            lbl_KichThuoc.Text = "Kích thước : ";
            // 
            // txt_KichThuoc
            // 
            txt_KichThuoc.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_KichThuoc.Location = new Point(15, 209);
            txt_KichThuoc.Name = "txt_KichThuoc";
            txt_KichThuoc.Size = new Size(558, 30);
            txt_KichThuoc.TabIndex = 80;
            // 
            // label5
            // 
            label5.AutoSize = true;
            label5.Font = new Font("Segoe UI", 13.8F, FontStyle.Italic, GraphicsUnit.Point);
            label5.Location = new Point(17, 226);
            label5.Name = "label5";
            label5.Size = new Size(0, 31);
            label5.TabIndex = 78;
            // 
            // lbl_ChiTietPhong
            // 
            lbl_ChiTietPhong.AutoSize = true;
            lbl_ChiTietPhong.Font = new Font("Segoe UI Semibold", 13.8F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_ChiTietPhong.Location = new Point(14, 70);
            lbl_ChiTietPhong.Name = "lbl_ChiTietPhong";
            lbl_ChiTietPhong.Size = new Size(181, 31);
            lbl_ChiTietPhong.TabIndex = 76;
            lbl_ChiTietPhong.Text = "Chi tiết phòng ở";
            lbl_ChiTietPhong.Click += lbl_ChiTietPhong_Click;
            // 
            // lbl_ThietLap
            // 
            lbl_ThietLap.AutoSize = true;
            lbl_ThietLap.Font = new Font("Segoe UI Semibold", 16.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_ThietLap.Location = new Point(9, 15);
            lbl_ThietLap.Name = "lbl_ThietLap";
            lbl_ThietLap.Size = new Size(218, 38);
            lbl_ThietLap.TabIndex = 75;
            lbl_ThietLap.Text = "Thiết lập phòng";
            // 
            // lbl_Nguoi
            // 
            lbl_Nguoi.AutoSize = true;
            lbl_Nguoi.BackColor = SystemColors.Window;
            lbl_Nguoi.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_Nguoi.Location = new Point(562, 761);
            lbl_Nguoi.Name = "lbl_Nguoi";
            lbl_Nguoi.Size = new Size(46, 23);
            lbl_Nguoi.TabIndex = 97;
            lbl_Nguoi.Text = "VNĐ";
            // 
            // txt_tienThemKhach
            // 
            txt_tienThemKhach.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_tienThemKhach.Location = new Point(16, 758);
            txt_tienThemKhach.Name = "txt_tienThemKhach";
            txt_tienThemKhach.Size = new Size(540, 30);
            txt_tienThemKhach.TabIndex = 96;
            // 
            // lbl_ThemKhach
            // 
            lbl_ThemKhach.AutoSize = true;
            lbl_ThemKhach.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_ThemKhach.ImageAlign = ContentAlignment.BottomRight;
            lbl_ThemKhach.Location = new Point(16, 716);
            lbl_ThemKhach.Name = "lbl_ThemKhach";
            lbl_ThemKhach.Size = new Size(131, 23);
            lbl_ThemKhach.TabIndex = 95;
            lbl_ThemKhach.Text = "Giá thêm khách";
            // 
            // lbl_VND
            // 
            lbl_VND.AutoSize = true;
            lbl_VND.BackColor = SystemColors.Window;
            lbl_VND.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_VND.Location = new Point(562, 676);
            lbl_VND.Name = "lbl_VND";
            lbl_VND.Size = new Size(46, 23);
            lbl_VND.TabIndex = 94;
            lbl_VND.Text = "VNĐ";
            // 
            // txt_GiaToiThieu
            // 
            txt_GiaToiThieu.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_GiaToiThieu.Location = new Point(17, 673);
            txt_GiaToiThieu.Name = "txt_GiaToiThieu";
            txt_GiaToiThieu.Size = new Size(539, 30);
            txt_GiaToiThieu.TabIndex = 93;
            // 
            // lbl_GiaToiThieuMoiDem
            // 
            lbl_GiaToiThieuMoiDem.AutoSize = true;
            lbl_GiaToiThieuMoiDem.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_GiaToiThieuMoiDem.Location = new Point(16, 647);
            lbl_GiaToiThieuMoiDem.Name = "lbl_GiaToiThieuMoiDem";
            lbl_GiaToiThieuMoiDem.Size = new Size(177, 23);
            lbl_GiaToiThieuMoiDem.TabIndex = 92;
            lbl_GiaToiThieuMoiDem.Text = "Giá tối thiểu mỗi đêm";
            // 
            // lbl_GiaGocChoPhongTieuChuan
            // 
            lbl_GiaGocChoPhongTieuChuan.AutoSize = true;
            lbl_GiaGocChoPhongTieuChuan.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_GiaGocChoPhongTieuChuan.Location = new Point(17, 610);
            lbl_GiaGocChoPhongTieuChuan.Name = "lbl_GiaGocChoPhongTieuChuan";
            lbl_GiaGocChoPhongTieuChuan.Size = new Size(242, 23);
            lbl_GiaGocChoPhongTieuChuan.TabIndex = 91;
            lbl_GiaGocChoPhongTieuChuan.Text = "Giá gốc cho phòng tiêu chuẩn";
            // 
            // lbl_GiaMoiDem
            // 
            lbl_GiaMoiDem.AutoSize = true;
            lbl_GiaMoiDem.Font = new Font("Segoe UI Semibold", 13.8F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_GiaMoiDem.Location = new Point(17, 554);
            lbl_GiaMoiDem.Name = "lbl_GiaMoiDem";
            lbl_GiaMoiDem.Size = new Size(146, 31);
            lbl_GiaMoiDem.TabIndex = 89;
            lbl_GiaMoiDem.Text = "Giá mỗi đêm";
            // 
            // txt_SoGiuong
            // 
            txt_SoGiuong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_SoGiuong.Location = new Point(18, 440);
            txt_SoGiuong.Name = "txt_SoGiuong";
            txt_SoGiuong.Size = new Size(593, 30);
            txt_SoGiuong.TabIndex = 88;
            // 
            // lbl_ThemHinhAnh
            // 
            lbl_ThemHinhAnh.AutoSize = true;
            lbl_ThemHinhAnh.Font = new Font("Segoe UI Semibold", 13.8F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_ThemHinhAnh.Location = new Point(721, 70);
            lbl_ThemHinhAnh.Name = "lbl_ThemHinhAnh";
            lbl_ThemHinhAnh.Size = new Size(283, 31);
            lbl_ThemHinhAnh.TabIndex = 100;
            lbl_ThemHinhAnh.Text = "Thêm hình ảnh của phòng";
            // 
            // btn_ThemPhong
            // 
            btn_ThemPhong.Location = new Point(825, 516);
            btn_ThemPhong.Name = "btn_ThemPhong";
            btn_ThemPhong.Size = new Size(94, 29);
            btn_ThemPhong.TabIndex = 108;
            btn_ThemPhong.Text = "Thêm phòng";
            btn_ThemPhong.UseVisualStyleBackColor = true;
            btn_ThemPhong.Click += btn_ThemPhong_Click;
            // 
            // ptb_AnhPhong
            // 
            ptb_AnhPhong.BackColor = SystemColors.ButtonShadow;
            ptb_AnhPhong.Location = new Point(676, 172);
            ptb_AnhPhong.Name = "ptb_AnhPhong";
            ptb_AnhPhong.Size = new Size(391, 331);
            ptb_AnhPhong.TabIndex = 109;
            ptb_AnhPhong.TabStop = false;
            ptb_AnhPhong.Click += ptb_AnhPhong_Click;
            // 
            // lbl_TenPhong
            // 
            lbl_TenPhong.AutoSize = true;
            lbl_TenPhong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_TenPhong.Location = new Point(17, 111);
            lbl_TenPhong.Name = "lbl_TenPhong";
            lbl_TenPhong.Size = new Size(105, 23);
            lbl_TenPhong.TabIndex = 110;
            lbl_TenPhong.Text = "Tên phòng : ";
            // 
            // txt_TenPhong
            // 
            txt_TenPhong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_TenPhong.Location = new Point(16, 137);
            txt_TenPhong.Name = "txt_TenPhong";
            txt_TenPhong.Size = new Size(594, 30);
            txt_TenPhong.TabIndex = 111;
            // 
            // txt_SoPhong
            // 
            txt_SoPhong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            txt_SoPhong.Location = new Point(16, 512);
            txt_SoPhong.Name = "txt_SoPhong";
            txt_SoPhong.Size = new Size(594, 30);
            txt_SoPhong.TabIndex = 113;
            // 
            // lbl_SoPhong
            // 
            lbl_SoPhong.AutoSize = true;
            lbl_SoPhong.Font = new Font("Segoe UI Semibold", 10.2F, FontStyle.Bold, GraphicsUnit.Point);
            lbl_SoPhong.Location = new Point(17, 486);
            lbl_SoPhong.Name = "lbl_SoPhong";
            lbl_SoPhong.Size = new Size(98, 23);
            lbl_SoPhong.TabIndex = 112;
            lbl_SoPhong.Text = "Số phòng : ";
            lbl_SoPhong.Click += lbl_SoPhong_Click;
            // 
            // btn_ChinhSua
            // 
            btn_ChinhSua.Font = new Font("Segoe UI", 9F, FontStyle.Regular, GraphicsUnit.Point);
            btn_ChinhSua.Location = new Point(825, 516);
            btn_ChinhSua.Name = "btn_ChinhSua";
            btn_ChinhSua.Size = new Size(94, 29);
            btn_ChinhSua.TabIndex = 114;
            btn_ChinhSua.Text = "Chỉnh sửa";
            btn_ChinhSua.UseVisualStyleBackColor = true;
            btn_ChinhSua.Visible = false;
            btn_ChinhSua.Click += btn_ChinhSua_Click;
            // 
            // btn_Next
            // 
            btn_Next.Location = new Point(910, 576);
            btn_Next.Name = "btn_Next";
            btn_Next.Size = new Size(94, 29);
            btn_Next.TabIndex = 115;
            btn_Next.Text = "Next";
            btn_Next.UseVisualStyleBackColor = true;
            btn_Next.Visible = false;
            btn_Next.Click += btn_Next_Click;
            // 
            // btn_Back
            // 
            btn_Back.Location = new Point(721, 576);
            btn_Back.Name = "btn_Back";
            btn_Back.Size = new Size(94, 29);
            btn_Back.TabIndex = 116;
            btn_Back.Text = "Back";
            btn_Back.UseVisualStyleBackColor = true;
            btn_Back.Visible = false;
            btn_Back.Click += btn_Back_Click;
            // 
            // UPhong
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            Controls.Add(btn_Back);
            Controls.Add(btn_Next);
            Controls.Add(btn_ChinhSua);
            Controls.Add(txt_SoPhong);
            Controls.Add(lbl_SoPhong);
            Controls.Add(txt_TenPhong);
            Controls.Add(lbl_TenPhong);
            Controls.Add(ptb_AnhPhong);
            Controls.Add(btn_ThemPhong);
            Controls.Add(lbl_ThemHinhAnh);
            Controls.Add(lbl_Nguoi);
            Controls.Add(txt_tienThemKhach);
            Controls.Add(lbl_ThemKhach);
            Controls.Add(lbl_VND);
            Controls.Add(txt_GiaToiThieu);
            Controls.Add(lbl_GiaToiThieuMoiDem);
            Controls.Add(lbl_GiaGocChoPhongTieuChuan);
            Controls.Add(lbl_GiaMoiDem);
            Controls.Add(txt_SoGiuong);
            Controls.Add(lbl_ChiDemSoLuong);
            Controls.Add(lbl_SoLuongNguoi);
            Controls.Add(lbl_SoGiuong);
            Controls.Add(lbl_SucChua);
            Controls.Add(txt_SucChua);
            Controls.Add(lbl_m2);
            Controls.Add(lbl_KichThuoc);
            Controls.Add(txt_KichThuoc);
            Controls.Add(label5);
            Controls.Add(lbl_ChiTietPhong);
            Controls.Add(lbl_ThietLap);
            Name = "UPhong";
            Size = new Size(1115, 801);
            Load += Phong_Load;
            ((System.ComponentModel.ISupportInitialize)ptb_AnhPhong).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label lbl_ChiDemSoLuong;
        private Label lbl_SoLuongNguoi;
        private Label lbl_SoGiuong;
        private Label lbl_SucChua;
        private TextBox txt_SucChua;
        private Label lbl_m2;
        private Label lbl_KichThuoc;
        private TextBox txt_KichThuoc;
        private Label label5;
        private Label lbl_ChiTietPhong;
        private Label lbl_ThietLap;
        private Label lbl_SauKhiNhieuHon;
        private Label lbl_Nguoi;
        private TextBox txt_tienThemKhach;
        private Label lbl_ThemKhach;
        private Label lbl_VND;
        private TextBox txt_GiaToiThieu;
        private Label lbl_GiaToiThieuMoiDem;
        private Label lbl_GiaGocChoPhongTieuChuan;
        private Label lbl_VuiLong;
        private Label lbl_GiaMoiDem;
        private TextBox txt_SoGiuong;
        private Label lbl_ThemHinhAnh;
        private PictureBox ptb_Anh1;
        private PictureBox ptb_Anh2;
        private PictureBox ptb_Anh3;
        private PictureBox ptb_Anh4;
        private PictureBox ptb_Anh5;
        private PictureBox ptb_Anh6;
        private Button btn_ThemPhong;
        private PictureBox ptb_AnhPhong;
        private Label lbl_TenPhong;
        private TextBox txt_TenPhong;
        private TextBox txt_SoPhong;
        private Label lbl_SoPhong;
        private Button btn_ChinhSua;
        private Button btn_Next;
        private Button btn_Back;
    }
}
